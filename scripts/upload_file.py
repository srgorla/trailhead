...
class ProgressMonitor:
    """Monitor upload progress in a separate thread without slowing down the upload"""
    ...
    def monitor_progress(self):
        """Run in separate thread to monitor file being read"""
        while not self.stop_flag.is_set():
            try:
                # Check current file position
                # This is a workaround - we'll estimate based on time
                elapsed = time.time() - self.start_time
                
                # Print a heartbeat every 3 seconds
                if elapsed > 0:
                    print(f"  Upload in progress... Elapsed: {elapsed:.0f}s")
...
def upload_file_with_curl(instance_url, access_token, file_path, timeout_minutes=120):
    """
    Use curl subprocess for upload with real-time progress.
    Curl has built-in progress bar that doesn't slow down the upload.
    """
    ...
    try:
        ...
        curl_cmd = [
            ...
            '-w', '\n%{http_code}',
            '--progress-bar',  # Show curl's built-in progress bar
            '-o', '-'  # Output response to stdout
        ]
        
        print("Uploading file with curl progress bar...\n")
        start_time = time.time()
        
        # Run curl with real-time output
        result = subprocess.run(
            curl_cmd,
            capture_output=True,
            text=True,
...
def upload_file_requests_simple(access_token, instance_url, file_path, timeout_minutes=120):
    """
    Upload using requests library (fast but minimal progress).
    Uses heartbeat monitoring in separate thread.
    """
    ...
    try:
        print("Uploading file (heartbeat every 3 seconds)...\n")
...
if __name__ == "__main__":
    ...
    FILE_SIZE_MB = 1000
    TIMEOUT_MINUTES = 120
    USE_CURL = False  # Set to False to use requests library
    ...
    # Upload
    if USE_CURL:
        print("Using curl (has built-in progress bar, no speed penalty)\n")
        result = upload_file_with_curl(INSTANCE_URL, ACCESS_TOKEN, FILE_PATH, TIMEOUT_MINUTES)
    else:
        print("Using requests library (heartbeat monitoring, maximum speed)\n")
        result = upload_file_requests_simple(ACCESS_TOKEN, INSTANCE_URL, FILE_PATH, TIMEOUT_MINUTES)
