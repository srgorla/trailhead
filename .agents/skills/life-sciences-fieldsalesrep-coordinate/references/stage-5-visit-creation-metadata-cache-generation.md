...
Detailed commands for Steps 14–15 of the visit creation workflow.
...
Set **both** records to `ValidationCompleted` — the generate API in Step 15 rejects the
...
Extract the **parent record ID** from the debug log output — it is required as the `parentMetadataRecordId` in Step 15.
