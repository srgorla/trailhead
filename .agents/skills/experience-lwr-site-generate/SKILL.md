...
- [bootstrap-template-byo-lwr.md](references/bootstrap-template-byo-lwr.md) - Site creation, template defaults
- [configure-content-route.md](references/configure-content-route.md) - Route creation (custom/object pages)
- [configure-content-view.md](references/configure-content-view.md) - View creation/editing (custom/object pages)
- [configure-content-themeLayout.md](references/configure-content-themeLayout.md) - Theme layout creation + theme sync
- [configure-content-brandingSet.md](references/configure-content-brandingSet.md) - Branding with color patterns/WCAG
- [handle-component-and-region-ids.md](references/handle-component-and-region-ids.md) - **UUID generation (CRITICAL)** for component and region ids used in views and themeLayout.
- [handle-ui-components.md](references/handle-ui-components.md) - Component discovery, schemas, insertion, configuration
- [configure-guest-sharing-rules.md](references/configure-guest-sharing-rules.md) - **Guest sharing rules** (`sharingGuestRules`) for public sites — use for any request involving "guest sharing rule", "Site Guest User", or sharing object records with unauthenticated visitors
- [update-site-urls.md](references/update-site-urls.md) - **Updating site URLs** - URL architecture, workflow for updating `urlPathPrefix` in DigitalExperienceConfig, Network, and CustomSite
...
- [ ] **ALWAYS** read [bootstrap-template-byo-lwr.md](references/bootstrap-template-byo-lwr.md) within the skill directory. Do not proceed to the next step without loading the file.
...
- [ ] MUST read [configure-content-route.md](references/configure-content-route.md)
- [ ] MUST read [configure-content-view.md](references/configure-content-view.md)
- [ ] MUST read [handle-component-and-region-ids.md](references/handle-component-and-region-ids.md)

### Adding UI Components to Pages

**Steps** (Follow the steps sequentially. Do not skip any step before proceeding):

- [ ] MUST read [handle-ui-components.md](references/handle-ui-components.md) to add LWCs to LWR sites.
- [ ] MUST read [handle-component-and-region-ids.md](references/handle-component-and-region-ids.md) to handle id generation
- [ ] MUST read [configure-content-themeLayout.md](references/configure-content-themeLayout.md) if a component has one of the following requirements:
...
- [ ] MUST read [handle-ui-components.md](references/handle-ui-components.md)

### Creating Theme Layouts

**Steps** (Follow the steps sequentially. Do not skip any step before proceeding):

- [ ] **CRITICAL**:Before doing anything else, MUST Check with user whether this new theme layout reuses an existing theme layout Lightning web component or requires a new one. If it requires a new one, make sure to read [handle-ui-components.md](references/handle-ui-components.md) to create the new theme layout component before proceeding. DO NOT skip this step even if doing so would be faster or more efficient.
- [ ] MUST read [configure-content-themeLayout.md](references/configure-content-themeLayout.md).
- [ ] MUST read [configure-content-view.md](references/configure-content-view.md) if need to apply theme layout to pages
...
- [ ] MUST read [configure-content-brandingSet.md](references/configure-content-brandingSet.md) to configure background colors, foreground colors, button colors, and other branding colors that affect all pages.
...
- [ ] MUST read the reference doc related to the target content types if the doc exists. e.g., if modifying `sfdc_cms__route`, load [configure-content-route.md](references/configure-content-route.md).
- [ ] MUST read [handle-component-and-region-ids.md](references/handle-component-and-region-ids.md) if creating or modifying view or theme layout
...
- [ ] MUST read [configure-guest-sharing-rules.md](references/configure-guest-sharing-rules.md) and follow all steps there.
...
- [ ] MUST read [update-site-urls.md](references/update-site-urls.md) to understand the three-component architecture and URL update workflow
