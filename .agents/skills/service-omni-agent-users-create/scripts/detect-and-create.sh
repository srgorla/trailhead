...
REUSED_USERS=$(echo "$DETECT_JSON" | jq -c '.existing_users // []')
...
if [ "$INACTIVE_COUNT" -gt 0 ]; then
  INACTIVE_ACTION="$INACTIVE_COUNT agent slot(s) are occupied by DEACTIVATED users (see inactive_users). Their usernames are globally unique so they cannot be recreated - reactivate them in Setup → Users, or free the usernames, before those slots can be used."
fi
...
if [ "$MISSING_COUNT" = "0" ]; then
  if [ "$INACTIVE_COUNT" -gt 0 ]; then
    jq -n \
      ...
      --argjson reused_users "$REUSED_USERS" \
    ...
    action_required: (
      [
        (if $pw_reset_count > 0 then
          ($pw_reset_count | tostring) + " user(s) were created but could NOT have a password set automatically and cannot log in yet. Manually reset their passwords (Setup → Users → Reset Password, or `sf org open` → user record) before use. See users_needing_password_reset and CREDENTIALS.json."
        else empty end),
