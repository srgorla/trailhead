#!/usr/bin/env node
// Deterministic preflight classifier for the Fulfiller-agent create flow.
//
...
//      matches the target ("IT Service Fulfiller"), confirm it carries a non-empty
...
// <masterLabel> is the template's display label (default "IT Service Fulfiller").
//
...
// on a template-instantiated agent regardless of its DeveloperName — and the
// collected developerName is the last fallback (the guard for self-created agents,
// which carry a null AgentTemplate). `masterLabel` is echoed only for the report's
// display label; it is NOT an idempotency key (a display name is renamable and
// non-unique). See classify-agent-existence.mjs for why. (The template row's
...
function classifyStudio(res) {
  if (!res.ok) {
    return { hasAccess: null, signal: 'CANNOT-CONFIRM', reason: 'Studio access — no readable body (endpoint not wired / empty). A successful createAgent is authoritative.' };
  }
  const d = res.data;
  const errs = asErrorArray(d);
  if (errs.length) {
    if (isNotFoundError(d)) return { hasAccess: null, signal: 'CANNOT-CONFIRM', reason: 'Studio access → 404 (access-check gate not wired on this org). Treat as cannot-confirm; a successful createAgent is authoritative.' };
...
function classifyTemplate(res, masterLabel) {
  ...
  if (!match) {
    return {
      ...
      reason: items.length === 0
        ...
        : `No template with masterLabel "${masterLabel}" in data[] (got: ${items.map((i) => i?.masterLabel).filter(Boolean).join(', ') || 'none'}). Hand off to the readiness check.`,
    };
  }
  ...
  // botDefinitionId is a direct link for PRE-PROVISIONED agents; when it is null,
  // the template's own `id` (matched against `BotDefinition.AgentTemplate` in
  // Phase 2) still catches a pre-provisioned agent regardless of what DeveloperName
  // this skill would have guessed (see classify-agent-existence.mjs). Agents this
  // skill creates never stamp `templateName`, so they carry BOTH a null row
  // botDefinitionId AND a null AgentTemplate — the Phase-2 read falls back to the
  // collected DeveloperName key for that case.
...
const masterLabel = rawLabel && rawLabel.trim() ? rawLabel : 'IT Service Fulfiller';
...
// Studio CANNOT-CONFIRM (confirmed 404 gate) does not block — createAgent is authoritative.
let verdict;
...
process.stdout.write(JSON.stringify({
  ...
  template: {
    ...
    // agent) — not "no agent". `id` is the reliable catcher for a pre-provisioned
    // agent whose DeveloperName differs from this skill's collected guess: it
    // matches `BotDefinition.AgentTemplate`, the platform-stamped source template.
    // `masterLabel` is echoed only for the report's display label (NOT an
    // idempotency key). The workflow passes botDefinitionId, `id`, and the collected
    // developerName to classify-agent-existence.mjs from this single source.
