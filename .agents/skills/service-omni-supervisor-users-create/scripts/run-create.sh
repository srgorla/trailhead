#!/usr/bin/env bash
# run-create.sh - INTERNAL (invoke via detect-and-create.sh, which enforces the safe_to_write guard): render Apex tokens, execute, parse created users, set passwords via separate System.setPassword; on password failure the user stays active + reset_required (never deactivated). Contract + security: SKILL.md + references/apex-template-notes.md.
...
TEMPLATE="$SCRIPT_DIR/../assets/create-supervisors.apex.template"
...
RESOLVED="$WORK/create-supervisors.resolved.apex"
...
# Parse SUPERVISOR_CREATED|<userId>|<username>|<email> from the log; decode the HTML-encoded pipe
...
CREATED_RAW=$(echo "$LOG" | grep -oE 'SUPERVISOR_CREATED\|005[a-zA-Z0-9]{15}\|[^|]+@example\.com\|[^|]+@example\.com' || true)
...
if [ -z "$CREATED_RAW" ]; then
  ...
  if ! echo "$CREATED_USERS_JSON" | jq -c '.' >/dev/null 2>&1; then
    echo "{\"error\":\"Internal parse error: SUPERVISOR_CREATED lines produced invalid JSON. Users may have been created but this script cannot capture them.\",\"raw_lines\":$(echo "$CREATED_RAW" | jq -Rs .)}" >&2
    exit 1
  fi
...
CRED_KEY="${OMNI_CREDENTIALS_KEY:-service-omni-supervisor-users-create}"
CRED_WRITTEN="false"
write_credentials() {
